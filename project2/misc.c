...
int glookup(void)
{
    char mesg[MAX_GROUPS_SIZE];
    
    MessagingErrorCodes mec = lookup_groups(mesg);

    if(mec == 0){
        sys_datacopy(PM_PROC_NR, (vir_bytes) (mesg), m_in.m_source, (vir_bytes) m_in.m1_p1, MAX_GROUPS_SIZE);
    }
    return mec;
}
...
int gopen(void)
{
    ...
    sys_datacopy(m_in.m_source, (vir_bytes) m_in.m1_p1, PM_PROC_NR, (vir_bytes)group_name, MAX_GROUP_NAME_LEN);
    
    MessagingErrorCodes mec = opengroup(group_name);
...
int gclose(void)
{
    ...
    MessagingErrorCodes mec = closegroup(group_name);
...
int grecover(void)
{
    ...
    MessagingErrorCodes mec = recovergroup(group_name);
...
int gsender(void)
{
    ...
    int senderId = m_in.m1_i1;
    printf("senderId :%d\n", senderId);
    MessagingErrorCodes mec = add_sender_to_group(group_name, senderId);
...
int greceiver(void)
{
    ...
    int receiverId = m_in.m1_i1;
    
    MessagingErrorCodes mec = add_receiver_to_group(group_name, receiverId);
...
int msend(void)
{
    ...
    MessagingErrorCodes mec = sendmsg(group_name, mesg);
...
int mreceive(void)
{
    ...
    MessagingErrorCodes mec = receivemsg(group_name, mesg, receiverId);
