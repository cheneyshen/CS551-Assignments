 ...
 *        req_inodeviewer                *
 *===========================================================================*/
int req_inodeviewer(fs_e, inode_nr)
endpoint_t fs_e;
ino_t inode_nr;
{
  message m;

  printf("req_inodeviewer\n");
  /* Fill in request message */
  m.m_type = REQ_INODEVIEWER;
  m.m1_i1 = inode_nr;
 ...
 *        req_damageinode                   *
 *===========================================================================*/
int req_damageinode(endpoint_t fs_e, ino_t pinode, ino_t finode, char* fullpath)
{
  cp_grant_id_t grant_id;
  message m;
  
  printf("req_damageinode\n");
  /*grant access to fullpath*/
  size_t len = strlen(fullpath)+1;
  grant_id = cpf_grant_direct(fs_e, (vir_bytes) fullpath, len, CPF_READ);
  if(grant_id==-1)
    panic("req_damageinode: cpf_grant_direct failed");
 ...
 *        req_damageinode                   *
 *===========================================================================*/
int req_restoreinode(endpoint_t fs_e, ino_t pinode, char* filename)
{
  cp_grant_id_t grant_id;
  message m;
  
  printf("req_restoreinode\n");
  /*grant access to filename*/
  size_t len = strlen(filename)+1;
  grant_id = cpf_grant_direct(fs_e, (vir_bytes) filename, len, CPF_READ);
  if(grant_id==-1)
    panic("req_damageinode: cpf_grant_direct failed");
